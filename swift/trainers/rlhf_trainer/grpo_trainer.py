...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: InputsType) -> InputsType:
        ...
        inputs = self._generate_completions(inputs) ## XIE: 基于ref模型采样
        ...
        # Prepare final outputs with advantages and other required fields XIE: 计算advantages 以及 所有的Pi_{theta}(o_t|q,o_{<t})
    ...
    def training_step(self, model: nn.Module, inputs: InputsType, num_items_in_batch=None) -> torch.Tensor:
        if self.args.async_generate:
            ...
            while not self.is_async_generate_eval_rollout_done():
                time.sleep(0.1)
        return super().training_step(model, inputs, num_items_in_batch) ## XIE: vscode堆栈看不到系统挤成库的调用，后面实际调用了2个重要的函数：_prepare_batch_inputs 和 compute_loss
